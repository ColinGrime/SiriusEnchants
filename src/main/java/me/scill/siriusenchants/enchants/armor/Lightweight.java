...
public class Lightweight extends CustomEnchant {
	...
	public void onArmorEquip(ArmorEquipEvent event, int level, boolean isOn) {
		...
		if (isOn) {
			switch (level) {
				case 1:
					player.addPotionEffect(CommonUtil.createPotionEffect(PotionEffectType.JUMP, 9999999, 1));
					break;
				case 2:
					player.addPotionEffect(CommonUtil.createPotionEffect(PotionEffectType.JUMP, 9999999, 2));
					break;
			}
		} else
